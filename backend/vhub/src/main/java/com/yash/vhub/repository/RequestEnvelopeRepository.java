...
public interface RequestEnvelopeRepository extends JpaRepository<RequestEnvelope, Long> {
	...
	@Query("SELECT e FROM RequestEnvelope e INNER JOIN e.requester r WHERE e.requestStatus.status <> 'CLOSED' AND CONCAT(r.firstName, ' ', r.lastName) LIKE %:name% ORDER BY e.requestDate DESC")
	...
	@Query("SELECT e FROM RequestEnvelope e INNER JOIN e.requester r WHERE e.requestStatus.status = 'CLOSED' AND CONCAT(r.firstName, ' ', r.lastName) LIKE %:name% ORDER BY e.requestDate DESC")
	Page<RequestEnvelope> findByRequesterNameByRequestStatus_StatusIsClosedOrderByRequestDateDesc(@Param("name") String name, Pageable pageRequest );
